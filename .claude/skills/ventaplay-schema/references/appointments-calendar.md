...
**Notification trigger (mig `20260805120000`):** `trg_notificar_evento_cita` (AFTER INSERT OR UPDATE) inserts into `notificaciones` (mobile-app feed) — INSERT → `creada` (skips `metodo_agendamiento IN ('importacion','sobreagendar')`, mig `20260808120001`), estado→`cancelada` → `cancelada`, change of `estado`/`fecha_cita`/`hora_inicio`/`hora_fin`/`profesional_id` → `modificada`; ignores updated_at/google_meet_url/notas. Dedups against the twin trigger on `atenciones` via a 10s `(cita_id, tipo)` window (cita/atención estados are synced by the web app). See [tasks-misc.md](tasks-misc.md#notificaciones).
...
Returns `(cita_id, fecha_cita, hora_inicio, hora_fin)` for each cita in the range where `citas.profesional_id <> p_profesional_id` (another pro is primary) AND any of:
- (1) `atenciones.profesional_id = p_profesional_id` (derivar profesional — dueño de la atención), OR
- (2) a `consumos` row of that atencion has `profesional_id = p_profesional_id AND servicio_id IS NOT NULL AND producto_id IS NULL` (dueño de un consumo de servicio), OR
- (3) **a `consumo_ayudantes` row links P to a `consumos` of that atencion** (ayudante de un consumo — added `20260721120002_ayudantes_en_derivaciones.sql`). `SELECT DISTINCT` dedups a pro who is both owner and ayudante into one occupied block.

`SECURITY DEFINER`, granted to `anon|authenticated|service_role`. Added 2026-04-22; ayudantes branch added 2026-06 (mig `20260721120002`). **This function is the single source of "what ranges P occupies by derivation"** — `obtener_slots_con_incremento_v2`, `validar_capacidad_cita` y `obtener_proxima_hora_por_servicio_batch` la consumen, así que heredan el cambio (un ayudante de consumo ocupa el tiempo del profesional en cupos/slots exactamente como una derivación) sin tocarse.
...
**Per-weekday service allow-list (mig `20260807120000_enforce_servicios_por_dia.sql`):** cada bloque de horario del profesional lleva una lista `servicios` (`configuracion_json → horarios_disponibles → detalle_por_dia → <día>[i].servicios`, construida desde `planillas.servicios_disponibles`). El RPC la parseaba pero la DESCARTABA — un servicio asignado globalmente (`profesional_servicio`) era reservable cualquier día que el profesional trabajara. Ahora `bloques_parseados` conserva un bloque solo si su allow-list contiene TODOS los `p_servicio_ids` (`@> to_jsonb(p_servicio_ids)`). **Estricto:** lista vacía/ausente/no-array = nada reservable en ese bloque. Sin servicio pedido (browse, `p_servicio_ids` null/vacío) = filtro salteado. Cierra availability Y creación para todos los paths reales (get-availability, `manage-appointments/create.ts` re-ejecuta este RPC y lanza antes del insert, calendario web). **Deliberadamente NO se tocó `validar_capacidad_cita`** — también dispara en UPDATE OF estado, y agregar el gate ahí bloquearía cancelar/finalizar citas pre-fix cuyo (servicio, día) ya no es válido.

### obtener_proxima_hora_por_servicio_batch(p_profesional_id uuid, p_servicio_ids uuid[], p_fecha_inicio date, p_fecha_fin date, p_incremento_minutos int=15, p_duracion_default int=null, p_min_slot_start timestamptz=null)
...
**Duration-dedup + servicios-por-día (mig `20260808120000_proxima_hora_batch_dedup_servicios_por_dia.sql`):** (A) **perf** — el RPC materializaba el espacio completo de slots (servicios × días × slots) para quedarse con 1 fila por servicio vía `DISTINCT ON`; con ~82 servicios × ~55 días reventaba su `statement_timeout` local de 20s (57014). Todo el pipeline pesado depende solo de la ventana temporal, nunca de `servicio_id`, así que ahora se keyea por `duracion` (`duraciones_unicas`) y las dos cantidades genuinamente per-servicio (`cupo_servicio`, motivo `servicios_requeridos`) se re-aplican en un fan-out final: O(servicios×días×slots) → O(duraciones-distintas×días×slots). (B) **paridad** — hereda el gate de allow-list por día de `20260807120000`, pero PER SERVICIO (containment por bloque, no el `@> ALL` de v2, porque acá cada servicio se computa independiente). Assumption documentada: los bloques de un mismo día no se solapan. Rollback: re-aplicar `20260610130000`.
...
**Trigger:** `trigger_validar_capacidad_cita` wraps this function and is bound `BEFORE INSERT OR UPDATE OF profesional_id, fecha_cita, hora_inicio, hora_fin, estado ON citas FOR EACH ROW` (mig `20260308172649_cupos_citas.sql:429`). Fires regardless of role — service_role, anon, authenticated all hit it. `metodo_agendamiento IN ('importacion','sobreagendar')` short-circuits the trigger (mig `20260808120001_sobreagendar_metodo.sql`): `'importacion'` for bulk imports (AgendaPro), `'sobreagendar'` for the deliberate over-book modal — before this migration the modal reused `'importacion'`, making over-books indistinguishable from imports in data. **UPDATE semantics diverge since mig `20260830120004_sobreagendar_update_bypass.sql`**: `'importacion'` on UPDATE only skips validation when the schedule DIDN'T change (hora-invariante); `'sobreagendar'` on UPDATE skips it UNCONDITIONALLY, even when profesional/fecha/hora change — needed by the cupo-lleno modal on drag&drop / confirmed reagendar, which always changes the schedule. Per-user gate: `usuarios.use_sobreagenda` (mig `20260830120000`, default true; admin/manager forced ON in UI — see [auth-roles.md](auth-roles.md)). The CHECK `citas_metodo_agendamiento_check` now allows `manual|web|api|whatsapp|telefono|importacion|sobreagendar`. Both values also skip the `notificar_evento_cita()` INSERT notification (misma política anti-avalancha).
