...
> = {
  bookings: {
    ...
    description: "Exporta citas reservadas y bloqueadas por sucursal.",
    buttonLabel: "Seleccionar carpeta y exportar",
    helperText: "Genera archivos separados para reservas y bloqueos cuando existan datos.",
  },
  services: {
    label: "Servicios",
    description: "Exporta el catalogo de servicios en un solo Excel.",
...
export default function LoginForm({ initialData, onSubmit }: Props) {
  ...
  const exportCopy = EXPORT_COPY[exportType];
  const showBookingsOptions = exportType === "bookings";
  ...
  return (
    ...
    >
      <div className="mb-0" dangerouslySetInnerHTML={{ __html: LOGO_SVG }} />

      <div className="text-center -mt-1 space-y-2">
        <h1 className="text-xl font-semibold text-white/90">Exportador AgendaPro</h1>
        <p className="text-sm text-white/50">
          Selecciona el tipo de exportacion y luego la carpeta donde guardar los archivos.
        </p>
      </div>
      ...
      <input
        ...
        className="w-full px-4 py-2.5 rounded-lg bg-surface-light border border-border text-white placeholder-white/40 outline-none focus:border-brand-purple transition-colors"
      />
      ...
      <fieldset className="w-full">
        <legend className="text-sm text-white/60 mb-2">Tipo de exportacion</legend>
        <div className="flex flex-col gap-2">
          <RadioOption
            name="exportType"
            value="services"
            checked={exportType === "services"}
          ...
          <RadioOption
            name="exportType"
            value="professionals"
            checked={exportType === "professionals"}
          ...
          <RadioOption
            ...
            label={EXPORT_COPY.bookings.label}
            description={EXPORT_COPY.bookings.description}
          />
        </div>
      </fieldset>

      {showBookingsOptions ? (
        <div className="w-full rounded-2xl border border-border bg-surface px-4 py-4 space-y-4">
          <div>
          ...
          <fieldset>
            <legend className="text-sm text-white/60 mb-2">
            ...
            <div className="flex flex-col gap-2">
              <RadioOption
                name="bookingType"
                value="all"
                checked={bookingType === "all"}
                onChange={() => setBookingType("all")}
                label="Reservas y bloqueos"
                description="Genera ambos archivos cuando existan datos disponibles."
              />
              <RadioOption
                name="bookingType"
                value="reserved"
                checked={bookingType === "reserved"}
                onChange={() => setBookingType("reserved")}
                label="Solo reservas"
                description="Genera unicamente el archivo de reservas."
              />
              <RadioOption
                name="bookingType"
                value="blocked"
                checked={bookingType === "blocked"}
                onChange={() => setBookingType("blocked")}
                label="Solo bloqueos"
                description="Genera unicamente el archivo de bloqueos."
              />
            </div>
          </fieldset>
        </div>
      ) : (
        <div className="w-full rounded-2xl border border-border bg-surface px-4 py-4">
          <p className="text-sm text-white/75">{exportCopy.helperText}</p>
          <p className="text-xs text-white/45 mt-2">
            Esta exportacion no usa el campo de meses ni el filtro de reservas/bloqueos.
          </p>
        </div>
      )}
      ...
      >
        {submitting ? "Seleccionando carpeta..." : exportCopy.buttonLabel}
      </button>
    </form>
  );
}
      ...
      <div className="relative w-4 h-4 flex-shrink-0 mt-0.5">
        ...
        <div
          className={`w-4 h-4 rounded-full border-2 transition-colors ${
            checked ? "border-brand-purple bg-brand-purple" : "border-border"
          }`}
        >
          {checked && (
